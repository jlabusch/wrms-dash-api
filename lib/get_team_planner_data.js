...
function query_sheet(range, next){
    ...
    const sheet = process.env['GOOGLE_SHEET_ID'] || '1JhPBMXINl7R-igc2VW3QxkyRVq0cXGf5cjb72iFTjxU',
        url = 'https://sheets.googleapis.com/v4/spreadsheets/' + sheet +
                '/values/' + range + '?key=' + process.env['GOOGLE_API_KEY'];

    util.request({url: url}, parse_sheet(next));
}
...
// {
//   "meta": [
//     [ "Project team SLA hours per month",  "100" ],
//     [ "SRE SLA hours per month",           "40"  ],
//     [ "Consultant SLA hours per month",    "40"  ]
//   ],
//   "teams": {
//     "n7":  [ "Andy" ],
//     "mi6": [ "Charlie", "Jeff"  ],
...
module.exports = {
    fetcher: fetch_planner,
    handler: function(req, res, next, ctx){
