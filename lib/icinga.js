...
function main(cache_label, file_label, get_config, process_next_key){
...
function get_config(name_field, pattern_field){
...
function process_next_key(poller, formatter){
    function process(key_list, ctx, next, result_list = []){
        ...
        if (k){
            poller(k.name, ctx).then(icinga_json => {
                ...
                return process(key_list, ctx, next, result_list);
            }).catch(next);
        }else{
...
function make_poller(get_options){
